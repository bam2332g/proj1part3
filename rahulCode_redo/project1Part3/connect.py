...
if option==1:
	...
	stmt="UPDATE team SET probofwinningchmp=(%s) WHERE teamid=(%s) and leagueid=1"
	with open("/home/azureuser/project/nba/final/teamProb") as f:
		for line in f:
			try:
				cur.execute(stmt,(float(line.strip().split(",")[0])/100.0,int(line.strip().split(",")[1])))
			except:
				print "I didn't work"
				print sys.exc_info()[0]
				print((float(line.strip().split(",")[0]),int(line.strip().split(",")[1])))
				break
else:
	stmt="INSERT INTO sportsleague (leagueID,name,avgYrsOfExp,numOfPlyrOnTeam,avgAgeOfPlyr,avgAgeOfTeam,avgWinPct) VALUES (%s,%s,%s,%s,%s,%s,%s)"
	...
	stmt="UPDATE team SET probofwinningchmp=(%s) WHERE teamid=(%s) and leagueid=0"
	with open("/home/azureuser/project/nfl/final/teamProb") as f:
		for line in f:
			try:
				cur.execute(stmt,(float(line.strip().split(",")[0])/100.0,int(line.strip().split(",")[1])))
			except:
